...
<div class="bg-white rounded-lg shadow-md overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-xl border-2 {{ $showResolved ? 'border-green-500' : 'border-gray-200' }}">
    <div class="p-4">
        <h3 class="text-xl font-semibold text-gray-800 mb-2 text-center">Found Matched!</h3>
        ...
        <div class="mt-4 border-t pt-4">
            ...
            <p class="text-md text-gray-800 truncate">
                ...
                @if ($item->lostItem->trashed()) {{-- Cek apakah di-soft-delete --}}
                    <span class="text-red-500 text-xs">(Sudah Dihapus)</span>
                @endif
            </p>
            <p class="text-sm text-gray-600">Lokasi: {{ $item->lostItem->location }}</p>
            {{-- Gambar: --}}
        ...
        <div class="mt-2 border-t pt-2">
            ...
            <p class="text-md text-gray-800 truncate">
                ...
                @if ($item->foundItem->trashed()) {{-- Cek apakah di-soft-delete --}}
                    <span class="text-red-500 text-xs">(Sudah Dihapus)</span>
                @endif
            </p>
            <p class="text-sm text-gray-600">Lokasi: {{ $item->foundItem->location }}</p>
            {{-- Gambar: --}}
