@php
    $showResolved = $item->resolved_date !== null;
@endphp
...
<div class="bg-white rounded-lg shadow-md overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-xl border-2 {{ $showResolved ? 'border-green-500' : 'border-gray-200' }}">
    <div class="p-4">
        ...
        <p class="text-sm text-gray-600 text-center">
            Matched Date: <span class="font-medium">{{ \Carbon\Carbon::parse($item->resolved_date)->format('d/m/Y') }}</span>
        </p>
        ...
        <div class="mt-4 border-t pt-4">
            ...
            <p class="text-md text-gray-800 truncate">
                {{ $item->lostItem->item_name ?? 'Item Lost Deleted' }}
                @if ($item->lostItem && $item->lostItem->trashed())
                    <span class="text-red-500 text-xs">(Deleted)</span>
                @endif
            </p>
            <p class="text-sm text-gray-600">Location: {{ $item->lostItem->location ?? 'N/A' }}</p>
        ...
        <div class="mt-2 border-t pt-2">
            ...
            <p class="text-md text-gray-800 truncate">
                {{ $item->foundItem->item_name ?? 'Item Found Deleted' }}
                @if ($item->foundItem && $item->foundItem->trashed())
                    <span class="text-red-500 text-xs">(Deleted)</span>
                @endif
            </p>
            <p class="text-sm text-gray-600">Location: {{ $item->foundItem->location ?? 'N/A' }}</p>
